  ...
  const checkAccount = useCallback(async () => {
    try {
      ...
      const res = await contract.methods
        .canIReport(props.brandDetail.brandOwner, accounts[0])
        .call();
      console.log('res', res);
      setBtnDetail(oldDetail => {
        return {
          ...oldDetail,
          enabled: res.canI,
          msg: res.reason,
    ...
    <div class='bg-white rounded-lg border border-gray-200 shadow-md w-80 m-4'>
      ...
      <div class='p-5'>
        ...
        {btnDetail.enabled ? (
          ...
          <button class='inline-flex items-center py-2 px-3 text-sm font-medium text-center text-white bg-red-700 rounded-lg hover:bg-red-800 focus:ring-4 focus:ring-red-300'>
            {btnDetail.msg}
          </button>
        )}
      </div>
