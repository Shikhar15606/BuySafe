...
contract('Brand', accounts => {
  ...
  it('Create a Brand', async () => {
    const result = await BrandInstance.createBrand('Nike', {
      from: accounts[0],
    });
  ...
  it('Brand name must be unique', async () => {
    await BrandInstance.createBrand('Nike', { from: accounts[0] });
    await utils.shouldThrow(
      BrandInstance.createBrand('Nike', { from: accounts[1] })
    );
  });
  ...
  it('Owner can have only 1 Brand', async () => {
    await BrandInstance.createBrand('Nike', { from: accounts[0] });
    await utils.shouldThrow(
      BrandInstance.createBrand('Reebok', { from: accounts[0] })
    );
  });
});
