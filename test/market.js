...
contract('Market', accounts => {
  ...
  it('Only Owner can change price of product', async () => {
    ...
    await utils.shouldThrow(
      MarketInstance.setPrice(0, 5, { from: accounts[2] })
    );
  });
  ...
  it('Price must be less than MRP', async () => {
    ...
    await utils.shouldThrow(
      MarketInstance.setPrice(0, 2000, { from: accounts[1] })
    );
  });
  ...
  it('Buy Product', async () => {
    ...
    const result = await MarketInstance.buyProduct(0, {
      from: accounts[2],
      value: 10,
    });
    assert.equal(result.logs[0].args._productId, 0);
  ...
  it('Can not buy product from less balance', async () => {
    ...
    await utils.shouldThrow(
      MarketInstance.buyProduct(0, { from: accounts[2], value: 9 })
    );
  });
  ...
  it('Can only buy product if its available for sale', async () => {
    ...
    await utils.shouldThrow(
      MarketInstance.buyProduct(0, { from: accounts[2], value: 10 })
    );
  });
});
